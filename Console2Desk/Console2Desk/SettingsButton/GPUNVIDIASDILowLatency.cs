...
{
    internal class GPUNVIDIASDILowLatency
    {
        public static void ToggleLowLatency(ControlToggles controlNVIDIAgpuSDI, MessagesBoxImplementation messagesBoxImplementation)
        {
            ...
            Thread toggleThread = new Thread(() =>
            {
                try
                {
                    ...
                    bool isOn = controlNVIDIAgpuSDI.IsOn;
                                ...
                                foreach (string subKeyName in deviceKey.GetSubKeyNames())
                                {
                                    using (RegistryKey subKey = deviceKey.OpenSubKey(subKeyName))
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(deviceDesc) && deviceDesc.Contains("NVIDIA GeForce"))
                                        {
                                            ...
                                            // Se si tratta di una GPU NVIDIA GeForce
                                            if (isOn)
                                            {
